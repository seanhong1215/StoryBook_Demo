...
- Local demo app: `src/App.jsx`，現在是 Product A adoption demo，用來驗證產品端如何透過 public API 組出實際後台畫面
...
- `.storybook/`
  ...
  - `preview.jsx`: 全域 Storybook 參數、tokens/docs CSS 載入、排序與測試設定
- `src/index.js`
  - library public API
  - 匯入全域 token 與元件 CSS
  - 匯出所有對外元件
- `src/App.jsx`
  ...
  - 只能使用 `src/index.js` 的 public exports，避免繞過套件入口
- `src/theme/`
...
目前沒有 Redux、Pinia、Zustand 或 API cache。核心 library 不持有全域狀態。`src/App.jsx` 只有 demo 畫面需要的 local state。
...
2. 確認影響範圍：
   ...
   - `src/App.jsx` 是否要補進 Product A demo
...
- 修改檔案
  ...
  - `src/App.jsx`: 如需展示產品端導入流程，補入 local adoption demo
