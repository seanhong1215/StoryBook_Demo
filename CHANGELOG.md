...
`0.1.0` 之後三個半月的內部試用回饋。有新功能，也有**四處會改變既有行為**
（下方 Changed 裡標 ⚠️ 的四項），升級前請先讀那一節。
...
- **Showcase 的「可操作的營運主控台」**：把有行為的元件串成一條真實流程 ——
  ...
  其餘四個 showcase 是靜態版型，各自補上一塊說明牌，寫明產品線、
  這一頁在示範什麼、用了哪些元件。
- **Showcase 的封面改用真元件重做**：原本是一張 PNG，圖裡畫的 Button 與色票
  跟真的元件對不起來 —— 對一個賣點是一致性的元件庫來說反而扣分。新的封面
  全部是真元件，色票讀 CSS 自訂屬性的實際計算值，而且不釘產品線：用工具列
  切明暗或換品牌，整張封面連同色票的十六進位數字都會重算。
  原本的 PNG 移到 README 當專案封面。
